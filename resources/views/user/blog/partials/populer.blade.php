...
<div class="card bg-base-100 shadow-lg border border-base-200" data-aos="fade-left" data-aos-delay="100">
    <div class="card-body">
        ...
        <div class="space-y-3">
            @foreach ($beritaPopuler as $populer)
                <article
                    class="flex items-start space-x-3 p-3 rounded-lg bg-base-200 hover:bg-base-300 transition-colors">
                    <div class="w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden">
                        <img src="{{ route('storage.show', ['path' => $populer->foto]) }}"
                            alt="{{ $populer->title }}" class="w-full h-full object-cover">
                    </div>
        ...
        <div class="mt-4 text-center">
            <a href="#" class="btn btn-outline btn-primary btn-sm">Lihat Semua</a>
        </div>
    </div>
</div>
