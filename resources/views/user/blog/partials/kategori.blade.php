        ...
        <div class="space-y-2">
            @forelse ($dataKategori as $kategori)
                <a href="{{ route('user.berita.kategori', $kategori->slug) }}"
                    class="flex justify-between items-center p-2 rounded-lg hover:bg-base-200 transition-colors">
                    <span class="flex items-center gap-3">
                        @if ($kategori->foto)
                            <img src="{{ route('storage.show', ['path' => $kategori->foto]) }}"
                                alt="{{ $kategori->nama }}" class="w-6 h-6 rounded object-cover">
                        @else
                            <div class="w-6 h-6 bg-base-300 rounded flex items-center justify-center">
